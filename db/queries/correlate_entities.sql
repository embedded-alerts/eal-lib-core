...
-- Cross-kind correlation for eal.page_embeddings: given one row, the rows of other kinds that are closest to it.
...
with src as (
  select e.id, e.workspace_id as tenant, e.model_key, e.embedding, e.embedding_bits,
    ...
    from eal.page_embeddings e
   where e.id = $2::uuid and e.workspace_id = $1::uuid
),
candidates as (
  select t.id
    from eal.page_embeddings t, src
...
select t.id, t.entity_kind, t.entity_id, t.title_text, t.summary_text, t.metadata,
  ...
  from candidates c
  join eal.page_embeddings t on t.id = c.id
  cross join src
