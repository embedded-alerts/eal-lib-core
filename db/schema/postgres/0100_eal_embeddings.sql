...
-- Postgres declarative schema for eal.page_embeddings (embeddings + hybrid search).
...
-- ---------------------------------------------------------------------
create table if not exists eal.embedding_models (
...
comment on table eal.embedding_models is
  'Registry of embedding models permitted in eal.page_embeddings. Vendored from the fleet embedding contract 2026.08.29; regenerate rather than hand-editing.';

insert into eal.embedding_models
...
create or replace function eal.emb_width() returns integer
  language sql immutable parallel safe
  as $$ select 4100 $$;

comment on function eal.emb_width() is
...
create or replace function eal.emb_pad(v extensions.vector)
...
create or replace function eal.emb_is_zero_padded(v extensions.vector, native_dims integer)
  returns boolean
  language sql immutable parallel safe
  as $$
    select case
...
create or replace function eal.emb_is_unit(v extensions.vector, tol double precision default 0.001)
  returns boolean
  language sql immutable parallel safe
  as $$
...
-- ---------------------------------------------------------------------
-- eal.page_embeddings
-- Turn the web into embeddings and run a semantic replacement for Google Alerts: a saved alert rule is itself an embedding, and a crawl fires when a new page or page section lands close enough to it.
-- ---------------------------------------------------------------------
create table if not exists eal.page_embeddings (
  id                uuid primary key default gen_random_uuid(),
  workspace_id      uuid not null,

  entity_kind       text not null
                      check (entity_kind in ('page', 'page_section', 'feed_item',
                                            'alert_rule', 'snapshot_diff',
                                            'extracted_claim')),
  ...
  model_key         text not null references eal.embedding_models (model_key),
  provider          text not null,
  model_name        text not null,
  model_version     text not null,
  ...
  constraint eal_emb_zero_padded
    check (eal.emb_is_zero_padded(embedding, native_dims)),
  constraint eal_emb_unit_norm
    check (eal.emb_is_unit(embedding, 0.001)),
  constraint eal_emb_model_key_is_derived
    check (model_key = provider || ':' || model_name || ':' || model_version),
  constraint eal_emb_one_row_per_entity_per_model
    unique (workspace_id, entity_kind, entity_id, model_key)
);

comment on table eal.page_embeddings is
  'Turn the web into embeddings and run a semantic replacement for Google Alerts: a saved alert rule is itself an embedding, and a crawl fires when a new page or page section lands close enough to it.';
comment on column eal.page_embeddings.embedding is
  'Unit-norm embedding, left-aligned and zero-padded to 4100. Never compare across model_key.';
comment on column eal.page_embeddings.embedding_bits is
...
comment on column eal.page_embeddings.embedding_prefix is
...
alter table eal.page_embeddings alter column embedding set storage external;
...
create index if not exists eal_emb_bits_hnsw_idx
  on eal.page_embeddings using hnsw (embedding_bits extensions.bit_hamming_ops);

-- Stage 1, higher recall, MRL models only.
create index if not exists eal_emb_prefix_hnsw_idx
  on eal.page_embeddings using hnsw (embedding_prefix extensions.halfvec_cosine_ops)
  where mrl_prefix_valid;

-- Lexical half of the hybrid query.
create index if not exists eal_emb_search_document_idx
  on eal.page_embeddings using gin (search_document);
...
create index if not exists eal_emb_scope_idx
  on eal.page_embeddings (workspace_id, model_key, entity_kind, generated_at desc);

-- Exact-duplicate short circuit: identical text under the same model.
create index if not exists eal_emb_content_sha_idx
  on eal.page_embeddings (workspace_id, model_key, content_sha256);

-- Reverse lookup from the owning entity, for re-embed and delete cascades.
create index if not exists eal_emb_entity_idx
  on eal.page_embeddings (workspace_id, entity_kind, entity_id);

create index if not exists eal_emb_metadata_idx
  on eal.page_embeddings using gin (metadata jsonb_path_ops);

-- ---------------------------------------------------------------------
-- updated_at maintenance
-- ---------------------------------------------------------------------
create or replace function eal.eal_emb_touch_updated_at()
...
drop trigger if exists eal_emb_touch_updated_at on eal.page_embeddings;
create trigger eal_emb_touch_updated_at
  before update on eal.page_embeddings
  for each row execute function eal.eal_emb_touch_updated_at();
