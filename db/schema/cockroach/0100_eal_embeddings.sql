...
-- CockroachDB declarative schema for eal.page_embeddings. Distributed deployments only.
...
create table if not exists eal.page_embeddings (
  ...
  entity_kind      string not null check (entity_kind in ('page', 'page_section', 'feed_item',
                                           'alert_rule', 'snapshot_diff',
                                           'extracted_claim')),
  entity_id        uuid not null,

  model_key        string not null references eal.embedding_models (model_key),
  provider         string not null,
  model_name       string not null,
  model_version    string not null,
  native_dims      int not null check (native_dims between 1 and 4100),
...
create inverted index if not exists eal_emb_search_document_idx
  on eal.page_embeddings (search_document);
...
-- CREATE VECTOR INDEX eal_emb_prefix_cspann_idx
--   ON eal.page_embeddings (workspace_id, model_key, embedding_prefix vector_l2_ops);
