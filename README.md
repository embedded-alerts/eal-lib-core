...
The single data boundary for `embedded-alerts`: `eal-web-server` and `eal-api-server` reach Postgres
...
| Database role | The DSN in `EAL_READONLY_DATABASE_URL` belongs to a role without `INSERT`, `UPDATE`, `DELETE` or DDL. |

A mistake has to defeat both. Depend on this crate from `eal-web-server` as:

```toml
eal-lib-core = { git = "https://github.com/embedded-alerts/eal-lib-core.git", default-features = false, features = ["read-only"] }
```

and from `eal-api-server` with `features = ["read-write"]`.
...
replace it with eal's real projections.
