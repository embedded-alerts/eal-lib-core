#![forbid(unsafe_code)]
//! Opaque, role-aware SeaORM boundary for eal.
//!
//! Both `eal-web-server` and `eal-api-server` reach Postgres through this crate
...
/// Named read operations, shared by `eal-web-server` and `eal-api-server`.
///
/// This is where eal's query builders go. Both servers call them, which
...
pub mod reads {
    ...
    ///
    /// Replace with eal's real projections; keep the shape — a named
...
mod tests {
    ...
    fn a_password_never_survives_redaction() {
        assert_eq!(
            redact_dsn("postgres://eal:hunter2@db.example.test:5432/eal?sslmode=require"),
            "postgres://eal:***@db.example.test:5432/eal?sslmode=require",
        );
        assert_eq!(
            redact_dsn("postgres://eal:p@ss:word@db.example.test/eal"),
            "postgres://eal:***@db.example.test/eal",
        );
        // Nothing to redact, nothing changed.
        assert_eq!(
            redact_dsn("postgres://db.example.test/eal"),
            "postgres://db.example.test/eal",
        );
    ...
    fn the_host_is_read_without_userinfo_or_port() {
        assert_eq!(
            dsn_host("postgres://user:pass@db.example.test:5432/eal"),
            Some("db.example.test"),
        );
        assert_eq!(dsn_host("postgres://localhost/eal"), Some("localhost"));
        assert_eq!(dsn_host("postgres://[::1]:5432/eal"), Some("[::1]"));
    }
    ...
    fn plaintext_is_refused_off_loopback_unless_the_url_asks_for_tls() {
        ...
        for mode in ENCRYPTED_SSL_MODES {
            let url = format!("postgres://u:p@db.example.test/eal?sslmode={mode}");
        ...
        let silent = "postgres://u:p@db.example.test/eal";
        assert!(
        ...
        for host in ["localhost", "127.0.0.1", "[::1]"] {
            let url = format!("postgres://u:p@{host}:5432/eal?sslmode=disable");
    ...
    async fn a_refused_url_fails_before_the_driver_sees_the_password() {
        let error = ReadContext::connect(
            "postgres://eal:hunter2@db.example.test/eal?sslmode=disable",
