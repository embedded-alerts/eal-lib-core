-- EmbeddedAlerts embedding contract v2 (PostgreSQL + pgvector)
...
CREATE OR REPLACE FUNCTION embedded_alerts.pad_embedding_4100(input REAL[])
RETURNS vector(4100)
LANGUAGE SQL
IMMUTABLE
STRICT
...
CREATE TABLE IF NOT EXISTS embedded_alerts.embedding_model_profiles (
...
INSERT INTO embedded_alerts.embedding_model_profiles
...
CREATE TABLE IF NOT EXISTS embedded_alerts.semantic_embeddings (
  ...
  purpose TEXT NOT NULL CHECK (purpose IN ('webpage_search', 'change_correlation', 'alert')),
  embedding_provider TEXT NOT NULL,
  ...
  FOREIGN KEY (embedding_provider, model)
    REFERENCES embedded_alerts.embedding_model_profiles (embedding_provider, model),
...
CREATE INDEX IF NOT EXISTS semantic_embeddings_scope_idx
  ON embedded_alerts.semantic_embeddings (tenant_id, embedding_space, purpose, entity_kind, embedded_at DESC);
CREATE INDEX IF NOT EXISTS semantic_embeddings_search_document_idx
  ON embedded_alerts.semantic_embeddings USING GIN (search_document);
...
ALTER TABLE embedded_alerts.semantic_embeddings FORCE ROW LEVEL SECURITY;
CREATE POLICY semantic_embeddings_tenant_isolation ON embedded_alerts.semantic_embeddings
  USING (tenant_id = current_setting('app.tenant_id', true)::UUID)
  WITH CHECK (tenant_id = current_setting('app.tenant_id', true)::UUID);

CREATE TABLE IF NOT EXISTS embedded_alerts.semantic_alert_rules (
...
ALTER TABLE embedded_alerts.semantic_alert_rules ENABLE ROW LEVEL SECURITY;
ALTER TABLE embedded_alerts.semantic_alert_rules FORCE ROW LEVEL SECURITY;
CREATE POLICY semantic_alert_rules_tenant_isolation ON embedded_alerts.semantic_alert_rules
  USING (tenant_id = current_setting('app.tenant_id', true)::UUID)
  WITH CHECK (tenant_id = current_setting('app.tenant_id', true)::UUID);

CREATE TABLE IF NOT EXISTS embedded_alerts.semantic_match_events (
  match_event_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  tenant_id UUID NOT NULL,
  alert_rule_id UUID NULL REFERENCES embedded_alerts.semantic_alert_rules (alert_rule_id) ON DELETE CASCADE,
  source_embedding_id UUID NOT NULL REFERENCES embedded_alerts.semantic_embeddings (embedding_id) ON DELETE CASCADE,
  candidate_embedding_id UUID NOT NULL REFERENCES embedded_alerts.semantic_embeddings (embedding_id) ON DELETE CASCADE,
...
ALTER TABLE embedded_alerts.semantic_match_events ENABLE ROW LEVEL SECURITY;
ALTER TABLE embedded_alerts.semantic_match_events FORCE ROW LEVEL SECURITY;
CREATE POLICY semantic_match_events_tenant_isolation ON embedded_alerts.semantic_match_events
  USING (tenant_id = current_setting('app.tenant_id', true)::UUID)
  WITH CHECK (tenant_id = current_setting('app.tenant_id', true)::UUID);
