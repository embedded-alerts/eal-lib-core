...
-define(FILEPATH, "src/eal_locks.gleam").
...
?MODULEDOC(
    " embedded-alerts lock routines: `ores_locks_and_leases` with the org's key prefix\n"
...
-file("src/eal_locks.gleam", 30).
?DOC(" Build `embedded-alerts/<domain>/<name>`.\n").
-spec key(domain(), binary()) -> {ok, ores_locks_and_leases:lock_key()} |
    {error, binary()}.
key(Domain, Name) ->
    ores_locks_and_leases:lock_key(
        <<<<<<<<"embedded-alerts"/utf8, "/"/utf8>>/binary,
                    (domain_to_string(Domain))/binary>>/binary,
                "/"/utf8>>/binary,
            Name/binary>>
    ).

-file("src/eal_locks.gleam", 54).
