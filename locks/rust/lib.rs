//! embedded-alerts lock routines.
//!
//! Re-exports [`ores_locks_and_leases`] and adds the org's key prefix and lock
//! catalog. Every key this crate builds is `embedded-alerts/<domain>/<name>`.
...
pub const ORG: &str = "embedded-alerts";
...
/// Build `embedded-alerts/<domain>/<name>`.
...
mod tests {
    ...
    fn keys_carry_the_org_prefix() {
        ...
        assert!(k.as_str().starts_with("embedded-alerts/jobs/"));
    }
    ...
    fn placeholders_are_filled_in_order() {
        ...
        assert_eq!(entry.key(&["1", "2"]).unwrap().as_str(), "embedded-alerts/jobs/1/x/2");
