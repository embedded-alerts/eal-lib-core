...
const pg = read('db/schema/postgres/0100_eal_embeddings.sql');
const crdb = read('db/schema/cockroach/0100_eal_embeddings.sql');
...
test('the zero-pad and unit-norm invariants are enforced by CHECK constraints', () => {
  assert.match(pg, /constraint eal_emb_zero_padded/);
  assert.match(pg, /constraint eal_emb_unit_norm/);
